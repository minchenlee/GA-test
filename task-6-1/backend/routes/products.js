...
router.get("/search", validateSearchParams, async (req, res) => {
  ...
  try {
      ...
      ...[
        {
          text: {
            query: features,
            path: ["productName", "productSpec.Features"],
            fuzzy: {},
          },
        },
      ],
    ...
    if (minScreenSize && maxScreenSize) {
      aggregationPipeline.push({
        $match: {
          "productSpec.Screen Size": {
            $gte: parseInt(minScreenSize),
            $lte: parseInt(maxScreenSize),
          },
        },
      });
    }
  ...
  try {
    ...
    if (!sortBy) {
      ...
      let maxGroupSize = Math.max(
        ...groupResults.map((group) => group.products.length),
      );
    ...
    Object.keys(filteringCondition).forEach(
      (key) =>
        filteringCondition[key] === undefined && delete filteringCondition[key],
    );
