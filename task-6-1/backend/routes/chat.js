  ...
  try {
    ...
    if (threads.length === 3) {
      return res.status(400).send({
        error: "Maximum number of threads reached: max 3 threads per user",
      });
    }
...
router.post("/messages", async (req, res) => {
  ...
  try {
      ...
      try {
        ...
        if (run.status === "requires_action") {
          ...
          result_list = result_list.map((result) => {
            ...
            if (
              result.tool_call_name === "get_product_details" ||
              "get_product_reviews"
            ) {
              delete result.tool_call_name;
              ...
              if (result.output.productReview) {
                result.output.productReview =
                  result.output.productReview.reviewList;
              }
            }
        ...
        console.error(
          `Error checking run status for thread ${threadId}:`,
          error,
        );
    ...
    if (run.status === "completed") {
          ...
          actionResult.map(async (result) => {
            ...
            if (isProductToolCall) {
              ...
              if (result.tool_call_name === "filter_products" && !noResult) {
                productImageList = result.output.map(
              ...
              if (result.tool_call_name === "filter_products" && noResult) {
                ...
                Object.keys(result.output[0]).includes("filteringCondition")
                  ? (metaData.filtering_condition = JSON.stringify(
                      result.output[0].filteringCondition,
                    ))
                  : {};
              ...
              if (productImageList) {
                ...
                imageList.map((imgSubList, index) => {
                  metaData[`img_list_${index + 1}`] =
                    JSON.stringify(imgSubList);
                });
                ...
                if (result.tool_call_name === "filter_products") {
                  ...
                  metaData.product_name_list = JSON.stringify(
                    result.output.map((product) => {
                  ...
                  metaData.product_id_list = JSON.stringify(
                    result.output.map((product) => product._id),
                  );

                  // Add the product price list to the metaData
                  metaData.product_price_list = JSON.stringify(
                    result.output.map((product) => product.productPrice),
                  );

                  // Add the product rating list to the metaData
                  metaData.product_rating_list = JSON.stringify(
                    result.output.map((product) => product.productRating),
                  );

                  // Add the product review count list to the metaData
                  metaData.product_review_count_list = JSON.stringify(
                    result.output.map((product) => product.productReviewCount),
                  );
                }
              ...
              if (result.tool_call_name === "get_product_reviews") {
                ...
                review.ratingDistributionList =
                  review.ratingDistributionList.map((rating) => {
                    delete rating.id;
                    return rating;
                  });
      ...
      res
        .status(200)
        .send({ success: true, message: "Message processed successfully" });
    } else {
      // Handle non-completion (either failed or max attempts reached)
      throw new Error(
        `Processing incomplete for thread ${threadId}: Status ${run.status}`,
      );
    }
  } catch (error) {
