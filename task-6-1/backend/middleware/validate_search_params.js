...
function validateSearchParams(req, res, next) {
  ...
  if (!keyword && !forFilter) {
    return res
      .status(400)
      .send({ error: "Missing parameter: keyword is required." });
  }
  ...
  if (limit && isNaN(parseInt(limit))) {
    return res
      .status(400)
      .send({ error: "Invalid parameter: limit must be a number." });
  }

  // limit must be less than or equal to 30
  if (limit > 30) {
    return res.status(400).send({
      error: "Invalid parameter: limit must be less than or equal to 30.",
    });
  }
  ...
  if (
    ...
    return res.status(400).send({
      error: "Invalid parameter: minPrice and maxPrice must be numbers.",
    });
  }
  ...
  if (parseInt(minPrice) > parseInt(maxPrice)) {
    return res.status(400).send({
      error:
        "Invalid parameter: minPrice must be less than or equal to maxPrice.",
    });
  }

  // minSreenSize and maxScreenSize validation
  if (
    (minScreenSize && isNaN(parseInt(minScreenSize))) ||
    (maxScreenSize && isNaN(parseInt(maxScreenSize)))
  ) {
    return res.status(400).send({
      error:
        "Invalid parameter: minScreenSize and maxScreenSize must be numbers.",
    });
  }
  ...
  if (parseInt(minScreenSize) > parseInt(maxScreenSize)) {
    return res.status(400).send({
      error:
        "Invalid parameter: minScreenSize must be less than or equal to maxScreenSize.",
    });
  }
  ...
  if (sortBy && !["price", "rating", "popularity"].includes(sortBy)) {
    return res.status(400).send({
      error:
        'Invalid parameter: sortBy must be one of "price", "rating", or "popularity".',
    });
  }
  ...
  if (fullDetails && fullDetails !== "true" && fullDetails !== "false") {
    return res
      .status(400)
      .send({ error: "Invalid parameter: fullDetails must be a boolean." });
  }
  ...
  for (let param of supportedFilterParams) {
    if (params[param] && typeof params[param] !== "string") {
      return res
        .status(400)
        .send({ error: `Invalid parameter: ${param} must be a string.` });
    }
  }
  ...
  if (params.length === 0 && forFilter) {
    return res.status(400).send({
      error: `Invalid parameter: at least one filter parameter is required.`,
    });
  }
  ...
  const unsupportedParams = Object.keys(params).filter(
    (param) => !supportedParams.includes(param),
  );
  if (unsupportedParams.length > 0) {
    return res.status(400).send({
      error: `Unsupported parameter(s): ${unsupportedParams.join(", ")}.`,
    });
  }
